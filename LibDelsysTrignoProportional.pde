...
*/
private LibDelsysTrignoBuffer myoBufferSingleton;
private LibDelsysTrignoBuffer getMyoBufferSingleton(PApplet mainObject) throws DelsysTrignoNotDetectedError {
  if (myoBufferSingleton == null)
    myoBufferSingleton = new LibDelsysTrignoBuffer(mainObject);

  return myoBufferSingleton;
}


class LibDelsysTrignoProportional {
  ...
  private LibDelsysTrignoBuffer myoBuffer;
  ...
  public LibDelsysTrignoProportional(PApplet mainObject) throws DelsysTrignoNotDetectedError {
  ...
  public void flushEmgData(long init_time,long end_time) {
    ConcurrentLinkedQueue<Sample> all = ((EmgManager)emgManager).pollAll();
    Sample[] windowSnapshot = {};
    windowSnapshot = all.toArray(windowSnapshot);
    PrintWriter output;
    final JFileChooser fc = new JFileChooser(sketchPath());
    int returnVal = fc.showOpenDialog(null);
    println(fc.getSelectedFile().getAbsolutePath());
    output = createWriter(fc.getSelectedFile().getAbsolutePath());
    for(Sample s: all){
      float[] data = s.sensorDataFloat;
      if( (s.timestamp > init_time) & (s.timestamp < end_time)){
        for(float i : data){
          output.print(i+"\t");
        }
        output.println();
