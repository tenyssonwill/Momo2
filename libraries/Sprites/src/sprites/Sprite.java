 ...
 * 
 * Unless otherwise specified when creating the sprite it will be registered with the library so it can
 * be updated with S4P.updateSprites(...) and drawn with S4P.drawSprites(). <br/>
...
public class Sprite implements PConstants, SConstants, Cloneable {
	 ...
	 * higher the value the nearer the viewer. <br/>
	 * 
	 * @param theApplet the PApplet
	 * @param imageFname filename for this sprite's image
	 ...
	 * @param theApplet the PApplet
	 * @param imageFname filename for this sprite's image
	 * @param alphaFname filename for the alpha mask file
	 * @param rows the number of vertical tiles
	 * @param zOrder the higher the z value the nearer the viewer
	 * @param register register the sprite with S4P needed to maintain z-order drawing
	 */
	 ...
	 * @param theApplet the PApplet
	 * @param imageFname filename for this sprite's image
	 * @param cols the number of horizontal tiles
	 * @param rows the number of vertical tiles
	 * @param zOrder the higher the z value the nearer the viewer
	 */
	...
	/**
	 * Ignore this method call if it does not cause a change in
	 * mouse responsiveness. <br>
	 ...
	 */
	public void respondToMouse(boolean mouse_respond){
		if(respondsToMouseEvents != mouse_respond){
			respondsToMouseEvents = mouse_respond;
...
//				y = app.mouseY;					
	...
	/**
	 * When set to true this sprite will be ignored by the updateremoved from the list of sprites
	...
	/**
	 * Comparator used for controlling the order sprites are drawn
	 * @author Peter Lager
	 */
	public static class Z_Order implements Comparator<Sprite> {

		public int compare(Sprite s1, Sprite s2) {
			return  s1.zOrder.compareTo(s2.zOrder);
		}
