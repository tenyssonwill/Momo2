...
public class S4P implements PConstants {
	...
	private static HashSet<Sprite> toAdd = new HashSet<Sprite>();
	private static HashSet<Sprite> toRemove = new HashSet<Sprite>();
	...
	/**
	 * Register a sprite so that it will be updated with S4P.updateSprites(...) and
	 * drawn with S4P.drawSprites().
	 * 
	 * @param sprite the sprite to be managed by this library
	 */
	 ...
	 * 
	 * @param sprite the sprite to be released from library control
	 */
	...
	public static void updateSprites(double deltaTime){
		...
		// Remove deregistered sprites
		if(!toRemove.isEmpty()){
			sprites.removeAll(toRemove);
			toRemove.clear();
		}
		// Add registered sprites
		if(!toAdd.isEmpty()){
	 ...
	 */
	static ImageInfo getImageInfo(PApplet theApplet, String imageFname, int ncols, int nrows){
		app = theApplet;
	 ...
	 * @return the ImageInfo object for this image file
	 */
	static ImageInfo getImageInfo(PApplet theApplet, String imageFname, String alphaMaskFname, int ncols, int nrows){
		app = theApplet;
		if(screenDomain == null)
			screenDomain = new Domain(0,0,app.width,app.height);
	 ...
	 */
	static PImage[] getFrames(ImageInfo info){
	 ...
	 * @return
	 */
	static PImage[] makeAlphaMaskFrames(ImageInfo info, int dw, int dh){
		int a,r,g,b;
